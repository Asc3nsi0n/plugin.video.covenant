...
class seasons:
    ...
    def seasonDirectory(self, items):
        ...
        views.setView('seasons', {'skin.estuary': 55, 'skin.confluence': 500, 'skin.tvOS-X-krypton': 53})
    ...
    def episodeDirectory(self, items):
        ...
        for i in items:
            try:
                ...
                url = '%s?action=play2&title=%s&year=%s&imdb=%s&tvdb=%s&season=%s&episode=%s&tvshowtitle=%s&premiered=%s&meta=%s&t=%s' % (sysaddon, systitle, year, imdb, tvdb, season, episode, systvshowtitle, syspremiered, sysmeta, self.systime)
                sysurl = urllib.quote_plus(url)

                path = '%s?action=play2&title=%s&year=%s&imdb=%s&tvdb=%s&season=%s&episode=%s&tvshowtitle=%s&premiered=%s' % (sysaddon, systitle, year, imdb, tvdb, season, episode, systvshowtitle, syspremiered)
        ...
        control.directory(syshandle, cacheToDisc=True)
        views.setView('episodes', {'skin.estuary': 55, 'skin.confluence': 504, 'skin.tvOS-X-krypton': 53})
