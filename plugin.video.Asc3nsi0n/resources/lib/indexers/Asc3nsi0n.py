...
import os,re,sys,hashlib,urllib,urllib2,urlparse,json,base64,random,cookielib,datetime
...
from resources.lib.modules import metacache2
...
class indexer:
	...
	def localXML(self, url, content):
		try:
			xmlDIR = 'special://home/addons/plugin.video.Asc3nsi0n/resources/xml/'
			#url = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.Asc3nsi0n', 'resources', 'xml', 'ENTERNAME.xml'))
			url = xmlDIR + url + '.xml'
			f = control.openFile(url) ; result = f.read() ; f.close()
			self.list = self.Asc3nsi0n_list('', result=result)
			for i in self.list: i.update({'content': content})
			self.addDirectory(self.list)
			return self.list
		except:
			pass

	def remoteXML(self, url, content):
		try:
			regex.clear()
			#url = base64.b64decode ('ENTERBASE64')
			self.list = self.Asc3nsi0n_list(url)
			for i in self.list: i.update({'content': content})
			self.addDirectory(self.list)
			return self.list
		except:
	...
	def root(self):
		try:
			self.remoteXML('http://ENTERURL/ENTERFILENAME.xml', 'addons')
		except:
			pass
	...
	def adult(self):
		try:
			...
			kb.setHeading(control.lang(33742).encode('utf-8')) # optional
			kb.setHiddenInput(True) # optional
			kb.doModal()
			if (kb.isConfirmed()):
				pw = control.setting('adult.pw')
				text = kb.getText()
				if pw in text:
					try:
						self.localXML('adult', 'addons')
					except:
						pass

				else:
					msg = control.lang(33756).encode('utf-8')
					dialog = xbmcgui.Dialog()
					ok = dialog.ok('Incorrect Password', msg)
		except:
			pass

	def xvideos(self):
		try:
			self.localXML('xvideos', 'addons')
		except:
			pass

	def ytplaylists(self):
		try:
			self.localXML('ytplaylists', 'addons')
		except:
			pass
	...
	def getx(self, url, worker=False):
		try:
			...
			url = regex.resolve(r)
			self.list = self.Asc3nsi0n_list('', result=url)
			self.addDirectory(self.list)
			return self.list
		except:
			pass
	...
	def developer(self):
		try:
			self.localXML('testing', 'addons')
		except:
			pass

	def kidsplace(self):
		try:
			self.localXML('kidsplace', 'addons')
		except:
			pass

	def kidspopular(self):
		try:
			self.localXML('kidspopular', 'addons')
		except:
			pass
	...
	def youtube(self, url, action):
		try:
			...
			if 'Tuner' in action:
				...
				for i in self.list: i.update({'name': i['title'], 'poster': i['image'], 'nextaction': action, 'action': 'play3', 'folder': False})
				self.addDirectory(self.list)
	...
	def addSearch(self, url):
			...
			self.list = [] ; threads = [] 
			for link in links: threads.append(workers.Thread(self.Asc3nsi0n_list, link))
			...
			for i in self.list:
				try:
					name = ''
					if not i['vip'] in ['Ascension']: name += '[B]%s[/B] | ' % i['vip'].upper()
					name += i['name']
					i.update({'name' : name})
				except:
					pass
	...
	def Asc3nsi0n_list(self, url, result=None):
		try:
	...
	def worker(self):
		...
		self.list = metacache2.fetch(self.list, self.lang)

		multi = [i['imdb'] for i in self.list]
		multi = [x for y,x in enumerate(multi) if x not in multi[:y]]
		if len(multi) == 1:
				self.movie_info(0) ; self.tv_info(0)
				if self.meta: metacache2.insert(self.meta)
	...
	def addDirectory(self, items, queue=False):
		...
		try: devmode = True if 'testing.xml' in control.listDir(control.dataPath)[1] else False
		...
		for i in items:
			try:
				...
				if content in ['movies', 'tvshows']:
					...
					cm.append((control.lang(33723).encode('utf-8'), 'RunPlugin(%s?action=trailer&name=%s)' % (sysaddon, urllib.quote_plus(name))))

				if content in ['movies', 'tvshows', 'seasons', 'episodes']:
					cm.append((control.lang(33724).encode('utf-8'), 'XBMC.Action(Info)'))

				#if content == 'movies':
				#	try: dfile = '%s (%s)' % (i['title'], i['year'])
				#	except: dfile = name
				#	try: cm.append((control.lang(32403).encode('utf-8'), 'RunPlugin(%s?action=addDownload&name=%s&url=%s&image=%s)' % (sysaddon, urllib.quote_plus(dfile), urllib.quote_plus(i['url']), urllib.quote_plus(poster))))
				#	except: pass
				...
				#	except: pass
				#elif content == 'songs':
				#	try: cm.append((control.lang(32403).encode('utf-8'), 'RunPlugin(%s?action=addDownload&name=%s&url=%s&image=%s)' % (sysaddon, urllib.quote_plus(name), urllib.quote_plus(i['url']), urllib.quote_plus(poster))))
				#	except: pass

				#if mode == 'movies':
				#	cm.append((control.lang(33725).encode('utf-8'), 'RunPlugin(%s?action=addView&content=movies)' % sysaddon))
				#elif mode == 'tvshows':
				#	cm.append((control.lang(33726).encode('utf-8'), 'RunPlugin(%s?action=addView&content=tvshows)' % sysaddon))
				#elif mode == 'seasons':
				#	cm.append((control.lang(33727).encode('utf-8'), 'RunPlugin(%s?action=addView&content=seasons)' % sysaddon))
				#elif mode == 'episodes':
				#	cm.append((control.lang(33728).encode('utf-8'), 'RunPlugin(%s?action=addView&content=episodes)' % sysaddon))
		...
		if mode in ['movies', 'tvshows', 'seasons', 'episodes']:
			views.setView(mode, {'skin.estuary': 55, 'skin.confluence': 500, 'skin.tvOS-X-krypton': 77})
...
class resolver:
	...
	def link(self, url):
		try:
			...
			if url == None: return control.infoDialog(control.lang(33722).encode('utf-8'))
			return url
		except:
			pass
	...
	def process(self, url, direct=True):
		...
		try:
			if control.setting('resolver') == '0': import resolveurl as urlresolver
			else: import urlresolver
...
def setxxxpass():
	d = xbmcgui.Dialog().yesno('Ascension','Would You Like To Set a Password for the XXX Section?')
	if d:
		...
		if (kb.isConfirmed()):
			pw = kb.getText()
			if pw =="":
				xbmcgui.Dialog().notification('[COLOR red]Password Cannot Be Blank[/COLOR]','Ascension')
				setxxxpass()
			else:
				xbmcaddon.Addon('script.module.Asc3nsi0n').setSetting('xxxpass',pw)
				xbmcaddon.Addon('script.module.Asc3nsi0n').setSetting('enablexxxpass','true')
				xbmcgui.Dialog().ok('Ascension','Password has been set')
