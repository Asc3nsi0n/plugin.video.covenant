...
class movies:
    def __init__(self):
        ...
        self.tm_user = control.setting('tmdb.user')
        self.fanart_tv_user = control.setting('fanart.tv.user')
        self.user = str(control.setting('fanart.tv.user')) + str(control.setting('tmdb.user'))
    ...
    def movieDirectory(self, items):
        ...
        for i in items:
            try:
                ...
                url = '%s?action=play2&title=%s&year=%s&imdb=%s&meta=%s&t=%s' % (sysaddon, systitle, year, imdb, sysmeta, self.systime)
                sysurl = urllib.quote_plus(url)

                path = '%s?action=play2&title=%s&year=%s&imdb=%s' % (sysaddon, systitle, year, imdb)
        ...
        views.setView('movies', {'skin.estuary': 55, 'skin.confluence': 500, 'skin.tvOS-X-krypton': 53})
