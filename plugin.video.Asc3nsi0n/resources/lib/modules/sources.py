...
class sources:
	...
	def addItem(self, title):
		...
		for i in range(len(items)):
			try:
				...
				###togglescraper###
				#if control.setting('scraper.default') == '1': cm.append((control.lang(33615).encode('utf-8'), 'RunPlugin(%s?action=togglescrapersources)' % (sysaddon)))
				#if control.setting('scraper.default') == '0': cm.append((control.lang(33614).encode('utf-8'), 'RunPlugin(%s?action=togglescrapersources)' % (sysaddon)))				
				###################
	...
	def getSources(self, title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, quality='HD', timeout=30):
		...
		progressDialog = control.progressDialog if control.setting('progress.dialog') == '0' else control.progressDialogBG
		progressDialog.create(control.addonInfo('name') + ' : ' + control.lang(33714).encode('utf-8'), '')
		progressDialog.update(0)
		...
		string1 = control.lang(33704).encode('utf-8')
		#string1 = control.lang(32404).encode('utf-8')###Time elapsed
		string2 = control.lang(33705).encode('utf-8')
		string3 = control.lang(32406).encode('utf-8')
		
		try: timeout = int(control.setting('scrapers.timeout.0'))
		except: pass

		#countdown time remaining
		for i in reversed(range(0, (timeout * 2))):
			try:
				if xbmc.abortRequested == True: return sys.exit()
				...
				try:
	#                    string4 = "[B]Getting Sources for : [/B]" 
	#                    string5 = "[B]%s (%s)[/B] - Time Elapsed: %s Seconds"  % (title,year,str(timerange))
					string4 = string1 % str(timerange)
					if len(info) > 7: string5 = string3 % str(len(info))
					else: string5 = string3 % str(info).translate(None, "[]'")
					progressDialog.update(int((100 / float(len(threads))) * len([x for x in threads if x.is_alive() == False])), string2 + str(sourcecount) + " | " + str(string4), str(string5))
				except:
					pass
		...
		for i in range(len(self.sources)):
			...
			if not d == '':
				if control.setting('prem.label') == 'true':
					label = '%02d | [COLOR %s][B]%s[/B][/COLOR] | ' % (int(i+1), prem_identify, p)#premium providers label by name
				else:
					label = '%02d | [COLOR %s][B]%s[/B][/COLOR] | ' % (int(i+1), prem_identify, d)#premium providers label is realdebrid
			else: label = '%02d | [B]%s[/B] | ' % (int(i+1), p)

			if multi == True and not l == 'en': label += '[B]%s[/B] | ' % l

			if t:
				if q in ['4K', '1440p', '1080p', '720p', 'HD']: label += '%s | [B][I]%s [/I][/B] | [I]%s[/I] | %s' % (s, q, t, f)
				elif q == 'SD': label += '%s | %s | [I]%s[/I]' % (s, f, t)
				else: label += '%s | %s | [I]%s [/I] | [I]%s[/I]' % (s, f, q, t)
			else:
				if q in ['4K', '1440p', '1080p', '720p', 'HD']: label += '%s | [B][I]%s [/I][/B] | %s' % (s, q, f)
	...
	def getLanguage(self):
		langDict = {'English': ['en'], 'German': ['de'], 'German+English': ['de','en'], 'French': ['fr'], 'French+English': ['fr', 'en'], 'Portuguese': ['pt'], 'Portuguese+English': ['pt', 'en'], 'Polish': ['pl'], 'Polish+English': ['pl', 'en'], 'Korean': ['ko'], 'Korean+English': ['ko', 'en'], 'Russian': ['ru'], 'Russian+English': ['ru', 'en'], 'Spanish': ['es'], 'Spanish+English': ['es', 'en'], 'Greek': ['gr'], 'Italian': ['it'], 'Italian+English': ['it', 'en'], 'Greek+English': ['gr', 'en'], 'Custom': ['en','yoda'], 'Debrid': ['debrid'], 'Yoda': ['yoda'], 'Exodus': ['exodus']}#added custom,debrid,yoda,& Exodus
		name = control.setting('provider.lang')
		return langDict.get(name, ['en'])

	def getLanguageWIP(self):
		english = 'en' if control.setting('providers.en') == 'true' else ''
		yoda = 'yoda' if control.setting('providers.yoda') == 'true' else ''
		german = 'de' if control.setting('providers.de') == 'true' else ''
		french = 'fr' if control.setting('providers.fr') == 'true' else ''
		polish = 'pl' if control.setting('providers.pl') == 'true' else ''
		korean = 'ko' if control.setting('providers.ko') == 'true' else ''
		russian = 'ru' if control.setting('providers.ru') == 'true' else ''
		spanish = 'es' if control.setting('providers.es') == 'true' else ''
		italian = 'it' if control.setting('providers.it') == 'true' else ''
		greek = 'gr' if control.setting('providers.gr') == 'true' else ''
		providers = [english, yoda, german, french, polish, korean, russian, spanish, italian,greek]
		#langDict = {'Yoda': ['yoda'],'English': ['en'], 'German': ['de'], 'German+English': ['de','en'], 'French': ['fr'], 'French+English': ['fr', 'en'], 'Portuguese': ['pt'], 'Portuguese+English': ['pt', 'en'], 'Polish': ['pl'], 'Polish+English': ['pl', 'en'], 'Korean': ['ko'], 'Korean+English': ['ko', 'en'], 'Russian': ['ru'], 'Russian+English': ['ru', 'en'], 'Spanish': ['es'], 'Spanish+English': ['es', 'en'], 'Greek': ['gr'], 'Italian': ['it'], 'Italian+English': ['it', 'en'], 'Greek+English': ['gr', 'en']}
		#name = control.setting('provider.lang')
		return providers
	...
	def getConstants(self):
		...
		self.hostcapDict = ['hugefiles.net', 'kingfiles.net', 'openload', 'openload.io', 'openload.co', 'oload.tv', 'thevideo.', 'thevideo', 'thevideo.me', 'vidup.me', 'streamin.to', 'torba.se']	
...
'''		
def togglescraper():
	if control.setting('internal.scrapers')=='false':
		control.setSetting(id='internal.scrapers', value='true')
		xbmc.executebuiltin("Container.Refresh")
	else:
		control.setSetting(id='internal.scrapers', value='false')
		xbmc.executebuiltin("Container.Refresh")

def togglescraper_sources():
	if control.setting('internal.scrapers')=='false':
		control.setSetting(id='internal.scrapers', value='true')
