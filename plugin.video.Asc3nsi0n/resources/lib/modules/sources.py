...
class sources:
    ...
    def play2(self, title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, meta, select):
        try:
            url = None
    ...
    def addItem(self, title):
		...
		#views.setView('files', {'skin.estuary': 55, 'skin.confluence': 504, 'skin.tvOS-X-krypton': 50})
    ...
    def getSources(self, title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, timeout=30):
        ...
        string2  = control.lang(33506).encode('utf-8')
...
#                    string5 = "[B]%s (%s)[/B] - Time Elapsed: %s Seconds"  % (title,year,str(timerange))
                    ...
                    progressDialog.update(int((100 / float(len(threads))) * len([x for x in threads if x.is_alive() == False])), string2 + str(countsources) + " | " + str(string4), str(string5))
                except:
                    pass
    ...
    def sourcesFilter(self):
        ...
        for i in range(len(self.sources)):
            ...
            if not d == '': label = '%02d | [COLOR %s][B]%s[/B][/COLOR] | ' % (int(i+1), debridcolor, d)#debrid sources label
