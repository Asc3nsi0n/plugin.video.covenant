...
#def artPath():
#    path = xbmc.translatePath('special://home/addons/plugin.video.Asc3nsi0n/resources/art')
#    return path
...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.Asc3nsi0n.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.Asc3nsi0n.artwork').getAddonInfo('path'), 'resources', 'media', theme)
		
		
def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.Asc3nsi0n.artwork)') else setting('appearance.alt').lower()
    return appearance
...
def metaFile():
    path = xbmc.translatePath('special://home/userdata/addon_data/plugin.video.Asc3nsi0n/meta')
    return path
